...
function maprender:update(dt)
    if love.keyboard.isDown("down") then
        self.y = math.max(self.y - 100 * dt, -self.maxHeight + self.h)
    end
    if love.keyboard.isDown("up") then
        self.y = math.min(self.y + 100 * dt, 0)
    end
    if love.keyboard.isDown("right") then
        self.x = math.max(self.x - 100 * dt, -self.maxWidth + self.w)
    end
    if love.keyboard.isDown("left") then
        self.x = math.min(self.x + 100 * dt, 0)
    end
...
function maprender:initialiseLayers()
    ...
    self.canvas = love.graphics.newCanvas(self.maxWidth, self.maxHeight)
